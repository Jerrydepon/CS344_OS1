...
// ============================ Main - Client =============================
// TCP Socket: http://zake7749.github.io/2015/03/17/SocketProgramming/
int main(int argc, char *argv[]) {
	int socketFD, portNumber;
	struct sockaddr_in serverAddress;
	struct hostent* serverHostInfo;
	int text_size, text_length, key_size;
	char t_buffer[71000], k_buffer[71000], c_buffer[142000], e_buffer[71000];
	...
	memset(e_buffer, '\0', sizeof(e_buffer)); // Clear out the buffer for encrypted data
		...
		for (i = 0; i < text_length; i++) {
			...
			// check character
				...
				if((ascii < 65 || ascii > 90) && ascii != 32 && ascii != 64) {
					error("Error: invalid character in plaintext (encrypt)\n");
					fclose(fp);
					exit(1);
				}
			}
	...
	// Send data to server for encryting
	int data_wirtten;
	int idx = 0;
	...
	// =========================================================
	// Read the encrypted data from server
	int data_read;
	int idx_2 = 0;
	int buffer2_size = sizeof(e_buffer) - 1;

	// receive data until e_buffer is filled
	while (idx_2 < sizeof(e_buffer) - 1) {
		data_read = recv(socketFD, e_buffer+idx_2, buffer2_size, 0);
		if (data_read < 0) {
	...
	// change the "@@" at the end of e_buffer into termination symbol
	int term_pos = strstr(e_buffer, "@@") - e_buffer;
	e_buffer[term_pos] = '\0';

	// print out the data from encryption if server is right
	printf("%s\n", e_buffer);
