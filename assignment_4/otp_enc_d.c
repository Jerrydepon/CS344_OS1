...
int main(int argc, char *argv[]) {
	...
	char c_buffer[142000], t_buffer[71000], k_buffer[71000], e_buffer[71000];
	...
	while(1) {
		...
		for(i = 0; i < strlen(t_buffer); i++) {
			...
			if(encrypt_i == 26)
				encrypt_i = 32;
			// encryption, transform back to 'A' to 'Z'
			else
				encrypt_i += 65;

			// encryption, transform back to char from int and store into e_buffer
			e_buffer[i] = (char)encrypt_i;
		}
		...
		while(idx_2 < sizeof(e_buffer) - 1) {
			data_wirtten = send(establishedConnectionFD, e_buffer+idx_2, buffer2_size, 0);
			if (data_wirtten < 0) {
